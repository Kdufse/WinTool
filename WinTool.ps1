...
Write-Host "———————————————————————————————————————————————"
Write-Host "[1]隐藏环境           [6]刷写KernelSU"
...
try {
    ...
    if ([System.Version]$remoteVersion -gt [System.Version]$localVersion) {
        ...
        $targetAsset = $releaseInfo.assets | Where-Object { $_.name -eq "WinTool.ps1" }
        
        if ($targetAsset) {
            $downloadUrl = $targetAsset.browser_download_url
            Invoke-WebRequest -Uri $downloadUrl -OutFile "WinTool.ps1"
            Write-Host "✅ 已更新至最新版本"
        } else {
...
Download-File -Url "https://hk.gh-proxy.org/https://github.com/Kdufse/WinTool/raw/refs/heads/main/Files/Modules.zip" -Output ".\Download\Modules.zip"
